...
class TestCore(unittest.TestCase):
    ...
    def test_max2(self):
        ...
        title = res.stdout.strip() # remove trailing newli
        self.assertEqual(title, "Settlement Remains From The Bronze And Iron Ages At Horbat Menorim (El-Manara), Lower Galilee")
    ...
    def test_eliot(self):
        ...
        res = command_with_output(["pdftitle", "-a", "eliot", "--eliot-tfs", "1", "-p", fpath, "--replace-missing-char", " "])
        self.assertEqual(res.returncode, 0)

        title = res.stdout.strip() # remove trailing newli
        self.assertEqual(title, "Lactobacillus HY2782 and Biﬁdobacterium HY8002 Decrease Airway Hyperresponsiveness Induced by Chronic PM2.5 Inhalation in Mice")
